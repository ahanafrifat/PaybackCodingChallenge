...
import com.ahanafrifat.paybackcodingchallenge.data.remote.FakePaybackApi
...
class HitRemoteMediatorTest {

    private lateinit var paybackApi: PaybackApi
    private lateinit var hits: List<Hit>

    @Before
    fun setUp() {
        paybackApi = FakePaybackApi()
        hits = listOf(
            Hit(
                id = 7885574,
    ...
    @Test
    fun `Search api with existing query, expect single hit result, assert LoadResult_Page`() {
        runTest {
//            val hitSource = HitRemoteMediator(paybackApi= paybackApi, query = "songbird", )
//            assertEquals<LoadResult<Int, Hit>>(
//                expected =LoadResult.Page(
//                    data = listOf(hits.first()),
//                    prevKey = null,
//                    nextKey = null
//                ),
//                actual= hitSource
